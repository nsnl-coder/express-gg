...
class CodeFilesHandler {
  ...
  checkDependencies() {
    ...
    const oneDependencies = {
      'yup-schemas': onePackageJsonContent.dependencies['yup-schemas'],
      express: onePackageJsonContent.dependencies.express,
      mongoose: onePackageJsonContent.dependencies.mongoose,
    };
    ...
    if (isMissingDependencies) {
      const missingDependencies = oneDependenciesKeys
        .filter((item) => !newDependenciesKeys.includes(item))
        .join(', ');
      ...
      console.log(
        chalk.blue(
          `Added missing dependencies to package.json: ${missingDependencies}`,
        ),
      );
    }
